...
Pasta e campo no markdown têm de coincidir. Sem `.gitkeep`. Não editar `agents/grok-bot/` — o gabinete atualiza os próprios links. `agents/procedures/` e `agents/templates/` são compartilhados.
...
4. Buscar no vault (exceto `agents/grok-bot/`) o caminho antigo e atualizar links externos. Links *dentro* da pasta do projeto (analysis, phases) não mudam.
