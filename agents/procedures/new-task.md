...
`recurring` com data: **Prazo:** `YYYY-MM-DD` = último dia. Hoje ≥ essa data → `completed`. Sem fim: **Prazo:** `none`. `none` é só recorrente sem fim — tarefa `pending` usa `não definido`.
...
3. Em [tasks](../../tasks/tasks.md): na seção do balde (ou **Recorrentes**), item `- [ ] [{Título}]({caminho a partir de tasks/})`. Trocar `--` se a seção estava vazia. `completed` não entra no índice. Checkbox só aqui.
